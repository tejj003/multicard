...
The supplied reference photographs informed the geometric vocabulary, not copied pixels or branding. All artwork is generated by the original fragment shader. No stock images or AI-generated assets are used in the displayed pieces.
...
Installed from https://github.com/MengTo/Skills into personal `~/.agents/skills/` and used: **threejs** and **3d-retina-resolution**. Applied their scene lifecycle, resizing, capped display-density, coordinate and verification practices. See DESIGN.md for the brief and THIRD_PARTY.md for dependency/model provenance.

## GitHub Pages

The installation has its own repository, `tejj003/multicard`, and Pages address: https://tejj003.github.io/multicard/.

The workflow in `.github/workflows/pages.yml` installs the locked dependencies on Node 22, prepares the local tracking assets, builds, and runs the Chromium/WebKit tests. A successful push to `main` or manual workflow run publishes only `dist` through GitHub Pages. Pull requests build and test without publishing. Select **GitHub Actions** as the Pages source in repository settings.

Source, shaders, the face model, fonts' dependency references and tests remain in the repository. Generated tracking runtime files, dependencies, build output, screenshots and private environment files are ignored. Relative asset URLs support the `/multicard/` path, including the favicon, camera worker and model. HTTPS allows opt-in camera access; opening the site never starts capture automatically.

To update the public installation, build and test locally, then commit and push the intended changes to this repository's `main` branch. Check the **Build and publish MultiCard** action before sharing the URL. No other project's deployment is affected.