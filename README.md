...
Choose Enable camera and approve the browser permission. Wait for **Body tracked / on-device**, then enter fullscreen. The first detected torso sets the centre. Moving the body sideways changes the view with mirror-style mapping and time-based smoothing. Holding still holds the angle; waving only a hand is not the input. When several bodies are visible, the largest confidently detected torso controls the artwork. It is a shared view, not independent interaction for each visitor.

MediaPipe Pose Landmarker Lite runs in a dedicated worker. Movement comes from the centre of the shoulders and hips, not a face box. All four torso landmarks must be confidently visible. Capture requests 1280x720 where supported; analysis frames preserve aspect ratio and are capped at 640 pixels wide, without upscaling lower-resolution cameras. Analysis is limited to 10 frames/second with one frame in flight. If the full image misses a body, up to three overlapping square regions are checked and their coordinates mapped back to the full camera view. This improves small-body detection in wide images but does not guarantee a particular distance or cover bodies that are too small, dark or obscured.

If no body is detected, the last angle is held. Pointer/touch fallback is available only in the gallery, never in fullscreen. Exit fullscreen to restart capture after stopping, denial or disconnection. Use Centre view in the gallery to recalibrate after moving the camera or changing viewer position. No face recognition or person identification is performed.
...
The body model is bundled at `public/tracking/pose-landmarker-lite.task` (~5.5MB). The MediaPipe runtime is copied from the pinned npm dependency during `predev`/`prebuild`; the copied WebAssembly directory is ~36MB across alternative builds, and only the selected variant is loaded after camera opt-in. It is not downloaded from a CDN at runtime. To restore a missing model, obtain the version-1 Pose Landmarker Lite model from the source listed in THIRD_PARTY.md.
...
3. Enable the camera, stand at the intended centre position and select Centre view. Wait for **Body tracked / on-device** and check that a step left/right changes the angle. If it stays on **finding body**, check camera framing and lighting, then test closer to the camera; a large screen alone does not establish a usable camera range.
