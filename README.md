...
2. Place a stable front-facing camera centrally on the display, with even frontal light and a single primary viewer. The short-range model is intended for nearby webcam viewing; distant visitors may need a different model or camera arrangement.
3. Enable the camera, stand at the intended centre position and select Centre view. Check that a small step left/right produces a clear transformation without reaching the angle limit too early.
...
Tests cover real canvas pixels, distinct left/right states for all six pieces, desktop/mobile/short layouts, keyboard/pointer/auto modes, reduced motion, Retina 1x/2x, accessibility, fullscreen, camera permission/cancellation/lifecycle and calibrated angle mapping. The three additions are also checked with simulated camera-position input, without reinitialising capture on artwork selection. Native fullscreen is verified in Chromium, not claimed for WebKit. A real local MediaPipe worker was checked against a public sample portrait at two positions and a browser-generated fake camera stream. No physical webcam was accessed during development.
